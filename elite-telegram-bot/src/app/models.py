...
class User(Base):
    ...
    ban: Mapped[Ban | None] = relationship("Ban", back_populates="user", uselist=False)
    ...
    telegram_profile: Mapped[TelegramProfile | None] = relationship("TelegramProfile", back_populates="user", uselist=False)
    badges: Mapped[list[UserBadge]] = relationship("UserBadge", back_populates="user")
    streak: Mapped[UserStreak | None] = relationship("UserStreak", back_populates="user", uselist=False)
...
class StripePrice(Base):
    ...
    recurring_interval: Mapped[str | None] = mapped_column(String(32))
...
class Product(Base):
    ...
    stripe_product_id: Mapped[str | None] = mapped_column(String(128), unique=True)
    ...
    unit_amount: Mapped[int | None] = mapped_column(Integer)
    recurring_interval: Mapped[str | None] = mapped_column(String(32))
    purchase_type: Mapped[str | None] = mapped_column(String(32))
    active: Mapped[bool] = mapped_column(Boolean, default=True)
    featured: Mapped[bool] = mapped_column(Boolean, default=False)
    telegram_slug: Mapped[str | None] = mapped_column(String(128))
    telegram_category: Mapped[str | None] = mapped_column(String(128))
    delivery_type: Mapped[str | None] = mapped_column(String(64))
    access_role: Mapped[str | None] = mapped_column(String(64))
    channel_announcement: Mapped[str | None] = mapped_column(Text())
    button_label: Mapped[str | None] = mapped_column(String(128))
...
class Purchase(Base):
    ...
    stripe_checkout_id: Mapped[str | None] = mapped_column(String(255), index=True)
    ...
    stripe_product_id: Mapped[str | None] = mapped_column(String(128), index=True)
...
class AccessGrant(Base):
    ...
    order: Mapped[Order | None] = relationship("Order", back_populates="grants")
